...
contract ExceptionsContract {
    ...
    function  revertBehavior(string memory name) public returns (bool) {
        ...
        // take a codnition, if not met => an exception is thrown. Require sent all gas back
        require(bytes(name).length > 0, "The length of the string is zero");

        return true;


        // earlier there was a throw statement, which used all gas instead of only the one used with revert
    }
    /*
