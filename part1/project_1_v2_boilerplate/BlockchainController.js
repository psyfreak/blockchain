...
class BlockchainController {
    ...
    getBlockByHeight() {
        this.app.get("/block/height/:height", async (req, res) => {
            if(req.params.height) {
                ...
                if(block){
                    // modification so that one can see humanreadable data
                    if(req.query.human != undefined) {
                        let readableBody = await block.getBData();
                        //console.log("human readable")
                        let modifiedBlock = JSON.parse(JSON.stringify(block))
                        modifiedBlock.body = readableBody;
                        return res.status(200).json(modifiedBlock);
                    } else {
    ...
    /**
     * This endpoint allows to validate the whole chain
     */
    validateChain() {
        this.app.get("/validation", async (req, res) => {
            try {
                ...
                if(errorArray.length === 0 ){
                    return res.status(200).json(errorArray);
                } else {
                    return res.status(404).send(errorArray);
                }
            } catch (error) {
                return res.status(500).send("An error happened!");
            }
    ...
    /**
     * This endpoint allows to show the chain
     */
    showChain() {
        this.app.get("/show", async (req, res) => {
            try {
                let data = {
