...
//  }
  ...
  // FIXME check if this is good reduce redundancy
  modifier changeOwner(uint _upc) {
    _;
  ...
  function buyItem(uint _upc) public payable 
    ...
    
    {
      ...
      items[_upc].ownerID = buyer;// FIXME add this to modifier Update current ownerId
      items[_upc].itemState = State.Sold;
      items[_upc].distributorID = buyer;

      // Transfer money to farmer
      address payable recipient = payable(items[_upc].originFarmerID);
      recipient.transfer(items[_upc].productPrice); //refund if sent more ether then the price is transferred to the recipient
  ...
  // FIXME should payable as buyItem
  ...
  // only current owner can
  function addHistoryEntry(
    uint _upc,
    string memory _txStr
  ) public
  // only current owner
